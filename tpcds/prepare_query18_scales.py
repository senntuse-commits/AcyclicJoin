#!/usr/bin/env python3
"""
Generate and export TPC-DS Q18 join-tree datasets by scale factor.

For each scale factor, this script ensures the required TPC-DS .dat files exist
and then exports integer projections consumed by main/SQL18TPCDSTest.cpp.
"""
...
from export_query18_join_tree import export_case, scale_label
...
def ensure_dsdgen(tools_dir: Path) -> Path:
    ...
    if not dsdgen.exists():
        ...
        # The repository may contain stale MinGW/Windows objects copied from a
        # Windows build. Linux gcc can see those .o files and then fail with
        # __mingw_* / __imp___acrt_iob_func link errors, so remove generated
        # binaries before rebuilding the TPC-DS tools natively.
...
def ensure_scale_data(tools_dir: Path, data_root: Path, scale: str, source_sf1: Path | None) -> Path:
    ...
    for table in REQUIRED_TABLES:
        if not (data_dir / f"{table}.dat").exists():
            generate_table(tools_dir, data_dir, scale, table)
    return data_dir


def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare TPC-DS Q18 projected datasets for sf1..sfN.")
    parser.add_argument("--scales", default="1,2,3,4,5", help="Comma-separated scale factors.")
    parser.add_argument("--tools-dir", default="tpcds/tools")
    parser.add_argument("--data-root", default="tpcds/generated")
    parser.add_argument("--out-root", default="tpcds/sql18_projected")
    parser.add_argument(
        "--source-sf1",
        default="tpcds/tools",
        help="Existing scale-1 data directory to reuse when it has required .dat files.",
    )
    ...
    for raw in args.scales.split(","):
        ...
        export_case(data_dir, out_root / scale_label(scale), scale, None, args.threshold)
    return 0
