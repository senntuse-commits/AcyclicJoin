#!/usr/bin/env python3
"""Generate and export red-box TPC-DS Q72 projected datasets by scale factor."""
...
from export_query72_join_tree import export_case, scale_label
...
def ensure_scale_data(tools_dir: Path, data_root: Path, scale: str, source_sf1: Path | None) -> Path:
    ...
    for table in REQUIRED_TABLES:
        if (data_dir / f"{table}.dat").exists():
            continue
        ...
        if not (data_dir / f"{table}.dat").exists():
            raise FileNotFoundError(
                f"missing {data_dir / f'{table}.dat'} after generating table {generator}"
            )
    return data_dir


def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare red-box TPC-DS Q72 projected datasets.")
